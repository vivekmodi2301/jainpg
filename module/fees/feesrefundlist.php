...
<?php
	$url="index.php?mod=fees&do=feeslist&";
		...
		$totRslt=mysqli_fetch_assoc($rs);
			$list=mysqli_query($con,"select student1.id, student1.name, fname, courses.coursename, subcourse.name as year,subcourse.fees from student1 left join courses on cid=courses.id left join subcourse on scid=subcourse.id $wh order by id desc LIMIT ".$frmdata['from'].", ".$frmdata['to']);
?>
            ...
            <tr>
                ...
                <td><?php 
					...
					if(mysqli_num_rows($fees_data)){
						...
						echo $studentdata['fees']-$totfee;
					}		
					else{
					echo $studentdata['fees'];
					}
				?></td>	
                <td><a href="index.php?mod=fees&do=feesrefundform&id=<?php echo $studentdata['id'];?>">Fees Refund</a></td>
            </tr>
            <?php	
		}
	?>
