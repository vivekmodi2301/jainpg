...
<table class="<?php echo $theam;?>_list">
	<tr>
    	<td colspan="<?php echo mysqli_num_rows($college_fund)+4;?>">Boys Funds List</td>
    </tr>
    ...
    <?php
	$rs=mysqli_query($con,"select distinct sid,name,collegefunddte.year,collegefunddte.date FROM collegefunddte join student1 on sid=student1.id join boysfund on funds=boysfund.id group by sid ");
	while($data=mysqli_fetch_assoc($rs)){?>
    <tr>
    <?php

			$list=mysqli_query($con,"SELECT boysfunddte.id, sid, date, funds, sum(boysfunddte.fees) as fees,name, boysfunddte.year,fund FROM boysfunddte join student1 on sid=student1.id join boysfund on funds=boysfund.fund where boysfunddte.sid=$data[sid] group by funds ");
		$i=1;?>
