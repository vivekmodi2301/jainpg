...
<table class="<?php echo $theam;?>_list">
    ...
    <?php
	$rs=mysqli_query($con,"select distinct sid,name,boysfunddte.year,boysfunddte.date FROM boysfunddte join student1 on sid=student1.id join boysfund on funds=boysfund.id group by sid ");
	while($data=mysqli_fetch_assoc($rs)){?>
    <tr>
    <?php

			$list=mysqli_query($con,"SELECT boysfunddte.id, sid, date, funds, boysfunddte.fees,name, boysfunddte.year,fund FROM boysfunddte join student1 on sid=student1.id join collegefund on funds=collegefund.id where boysfunddte.sid=$data[sid]");
