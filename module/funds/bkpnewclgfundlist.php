...
//		echo "SELECT tution_fees as tution_fees FROM studentfees WHERE cid=$data[cid] group by cid";
   ...
   <?php
	...
	$ch_rs=mysqli_query($con,"SELECT extraFees FROM studentfees where extraFees!=''");

	while($ch_data=mysqli_fetch_assoc($ch_rs))
	{
		$ch_array=explode(',',$ch_data['extraFees']);
//		print_r($ch_array);
		for($k=0;$k<=count($ch_array);$k++){
			$ch_name=explode('-',$ch_array[$k]);	
			//print_r($ch_name);
	...
	$rs=mysqli_query($con,"SELECT collegefunddte.cid as cid, collegefunddte.dt, collegefunddte.scid,count(distinct collegefunddte.sid) as total, coursename,year,sum(collegefunddte.fees) as gtotal,extraFees FROM collegefunddte join courses on courses.id=collegefunddte.cid join studentfees on studentfees.sid=collegefunddte.sid where extraFees!='' group by collegefunddte.cid");
	$count=mysqli_num_rows($rs);

	
	while($data=mysqli_fetch_assoc($rs) ){
		//echo count(explode(',',$data['extraFees']));
		//print_r($data['extraFees']);
	?>
    <tr>
        
		<td><?php echo $i++;?></td>
     	<td><?php echo $data['coursename'].'-'.$ch_name[$k];?></td>
     	<td><?php echo $data['total'];?></td>
        <?php
