...
//			echo $_POST['optsubject']; exit;
		...
		addEdit('student1',$_POST);
		?>
        <script>
			location.href="index.php?mod=register&do=studentUpdateClass";
		</script>
        <?php
	}
?>
...
<td>
	<input type="text" placeholder="Enter Student Name" name="name" value="<?php if(isset($studentdata['name']) && $studentdata['name']){echo $studentdata['name'];}?>" required >
</td>
</tr>
<tr>
	<td>Father Name</td>
    <td><input type="text" placeholder="Enter Student Father Name" value="<?php if(isset($studentdata['fname']) && $studentdata['fname']){echo $studentdata['fname'];}?>" name="fname"></td>
</tr>
...
<tr>
    ...
    <td><input type="text" readonly value="<?php $detail=mysqli_fetch_assoc(mysqli_query($con,"select student1.id, courses.coursename, subcourse.name, optsubject from student1 left join courses on student1.cid=courses.id left join subcourse on student1.scid=subcourse.id where student1.id=$id")); echo $detail['coursename'] ?>"><input type="tex" name="old_cid" value="<?php echo $studentdata['cid'];?>" hidden="hidden"></td>
</tr>	
<tr>
	<td><input type="text" readonly value="<?php echo $detail['name'];?>"><input type="hidden" value="<?php echo $studentdata['scid'];?>" name="old_scid" hidden="hidden"> </td>
</tr>
    
<tr>
	<td ><input type="text" readonly style="width:350px;" name="old_subject" value="<?php echo $detail['optsubject'];?>"></td>
</tr>
<?php }?>
<?php
if($id){?>
...
<td>
	...
	<td>Scholar Code</td>
    <td>
    <input type="text" readonly value="Scholar"/>
...
<tr>
	<td>BPL</td>
    <td><input type="radio" name="bpl" <?php if(isset($studentdata['bpl'])&&$studentdata['bpl']=='y'){?> checked<?php }?> value="y">Yes
    <input type="radio" name="bpl" value="n">No</td>
</tr>
<tr>
	<td>Phone No.</td>
    <td><input type="text" style="width:50px;" name="std" value="<?php if(isset($studentdata['std'])&&$studentdata['std']){echo $studentdata['std'];}else{?> 0151 <?php }?>"><input type="text" placeholder="Enter Student Phone no." value="<?php if(isset($studentdata['std'])&&$studentdata['std']){echo $studentdata['phone'];}?>" name="phone"></td>
</tr>
<tr>
	<td>Mobile No.</td>
...
<tr><td>Year</td>
<td>
<input type="text" placeholder="Year of passing last class" value="<?php if(isset($studentdata['year'])&&$studentdata['year']){echo $studentdata['year'];}?>" name="year"></td>
</tr>
<tr>
<td>Subject</td>
...
<tr><td>Total Marks</td>
     ...
     <tr>           <td>Percentage</td>
            ...
            <tr>
                ...
                <td><input type="text" placeholder="RBSE/CBSE" value="<?php if(isset($studentdata['board_name'])&&$studentdata['board_name']){echo $studentdata['board_name'];}?>" name="board_name"></td>
            </tr>
<tr>
<td colspan="2">
...
<script type="text/javascript">
	function mshow(chk,radioo,tot){
			var checkedCheckBoxes = $('#courses').find(':checkbox:checked');
